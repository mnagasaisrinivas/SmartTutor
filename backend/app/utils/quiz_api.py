...
class QuizQuestion(BaseModel):
    ...
    options: list[str] = Field(
        ..., description="List of exactly 4 strings representing options"
    )
...
prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "You are an expert tutor. Create a 5-question multiple choice quiz.",
        ),
        (
            "user",
...
def get_quiz_questions(subject: str, topic: str) -> Quiz:
    try:
        structured_quiz = chain.invoke({"subject": subject, "topic": topic})
        if not isinstance(structured_quiz, Quiz):
            raise ValueError(f"Unexpected response type: {type(structured_quiz)}")

        return structured_quiz
    except ValueError as e:
        logger.error(f"AI Generation failed for quiz_api: {e}", exc_info=True)
        raise AIGenerationError("Failed to generate valid quiz from AI.") from e
