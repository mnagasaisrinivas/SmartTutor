...
class StudyNotes(BaseModel):
    ...
    bullet_points: list[str] = Field(..., description="Key points to study")
...
def get_study_notes(subject: str, topic: str) -> StudyNotes:
    try:
        structured_notes = chain.invoke({"subject": subject, "topic": topic})
        if not isinstance(structured_notes, StudyNotes):
            raise ValueError(f"Unexpected response type: {type(structured_notes)}")

        return structured_notes
    except ValueError as e:
        logger.error(f"AI Generation failed for notes_api: {e}", exc_info=True)
        raise AIGenerationError("Failed to generate valid study notes from AI.") from e
